## Agriculture Output Products 

## PRD: Negotiation Process Overview
Negotiation in price involves a dynamic process where buyers and sellers engage in bid adjustments and counteroffers to reach a mutually agreeable price. Buyers initiate via an anchor price, and sellers respond with counterprices based on their own criteria and market conditions. This iterative exchange continues until both parties agree on terms or conclude the negotiation.


This document explains the sequence diagram depicting the interaction between a Buyer and a Seller during a product bidding process. It covers the steps involved from searching for a product to confirming an order. API interactions are marked with a `/` symbol, representing the respective API endpoints used in each step.

Participants

1. Buyer : The individual interested in purchasing a product through a bidding process.
2. Seller : The entity managing the products and overseeing the bidding process.


1. Exploratory Search for Product


Buyer initiates a search: The Buyer performs an exploratory search to find a product. This is done through the `/search` API.


Seller provides search results: The Seller responds with details about available products and their Base prices using the `/on_search` API.


2. Product Selection and Buyer Verification


Buyer selects a product: The Buyer selects a product and shares their Buyer ID using the `/select` API.


Seller verifies Buyer ID: The Seller verifies the Buyer ID internally to determine if the Buyer is eligible for bidding.


  Ineligible Buyer: If the Buyer is not eligible, the Seller informs the Buyer via the `/on_select` API that they are not eligible for the selected Product.


  Eligible Buyer: If the Buyer is eligible, the Seller confirms the Product details with the Buyer using the `/on_select` API.


3. Entering Anchor Price and Quantity


Buyer enters Anchor Price: The Buyer provides their Anchor Price and desired quantity through the `/select` API.


Seller updates product details: The Seller updates the product listing with a counter-price and quantity based on the Buyer's Anchor Price using the `/on_select` API.
The loop continues till the Buyer and Seller reaches an agreement.


4. Initializing and Confirming the Order


Buyer initializes the order: The Buyer initiates the order and proceeds to checkout after the negotiated (& agreed) Price and Quantity using the `/init` API.


Seller processes order initialization: The Seller's system processes the order initialization and responds using the `/on_init` API.


Buyer confirms the order: The Buyer confirms the order through the `/confirm` API.


Seller provides Order ID: The Seller provides an Order ID to the Buyer to complete the order process using the `/on_confirm` API.
